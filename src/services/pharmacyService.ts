...
export const pharmacyService = {
  ...
  getPharmacies: async () => {
    ...
    return pharmacies.map(pharmacy => ({
      ...pharmacy,
      contacts: contacts.filter(contact => contact.id_pharmacie === pharmacy.id),
      horaires: horaires.filter(horaire => horaire.id_pharmacie === pharmacy.id)
    }));
  },

  // Ajouter une nouvelle pharmacie avec ses contacts et horaires
  addPharmacy: async (pharmacyData: Omit<Pharmacy, 'id' | 'contacts' | 'horaires'>, 
